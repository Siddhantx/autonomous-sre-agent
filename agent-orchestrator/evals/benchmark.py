...
def write_benchmark(data: dict) -> None:
    lines = [
        ...
        "deterministic rules).",
        "",
        "> **Read this first.** The agent column is produced in `--fake-llm` "
        "mode: a `ScriptedLLM` replaying a pre-written transcript per scenario. "
        "Its sub-second time-to-diagnosis is transcript-replay speed, **not** "
        "reasoning speed, and its accuracy is the architecture ceiling, not a "
        "model score. Treat this as a pipeline smoke test — it is not a "
        "like-for-like comparison against a human SRE. For real model numbers "
        "see [`RESULTS-local-llm.md`](RESULTS-local-llm.md).",
        "",
