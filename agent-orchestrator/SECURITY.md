...
| Call 14 read-only tools (fixed queries authored in code; see below) | Author SQL — tools run queries written in `investigator.py`; the one exception, `pg_explain`, accepts only a single `SELECT`/`WITH` statement and wraps it in `EXPLAIN` (never `ANALYZE`) |
